...
def train_model(model, train_loader, val_loader, epochs=30):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "model/v3/best_model.pth")
            print("Model saved with improved validation loss.")

# Check if the model already exists
if not os.path.exists("model/v3/best_model.pth"):
...
model.load_state_dict(torch.load("model/v3/best_model.pth", weights_only=True))
model.eval()
...
def show_predictions(images, labels, predictions, class_names):
    ...
    for i, ax in enumerate(axes):
        ...
        pred_label = class_names[predictions[i]] if predictions[i] != "default" else "Unknown"
